...
- 🤖 **AI智能助手** - 基于DeepSeek-V3的SSE流式问答、RAG知识库检索、帖子向量检索、会话管理
...
| campushare-agent | 8083 | AI智能助手（SSE流式对话/知识库检索/会话管理） |
...
> **📋 架构升级规划**：项目正在进行专业架构升级，包括服务注册发现（Nacos）、消息队列（RocketMQ）、熔断限流（Sentinel）、分布式锁（Redisson）、多级缓存、Elasticsearch搜索、WebSocket实时通信、前端数据层重构（TanStack Query）、测试体系建设等。详见 [docs/architecture-upgrade/](./docs/architecture-upgrade/)。
...
#### 🤖 AI智能助手（agent-service）
- 基于DeepSeek-V3的SSE流式对话（打字机效果）
- RAG知识库检索增强（18篇平台使用文档向量化，pgvector存储）
- 帖子向量检索（用户提问可检索相关帖子内容）
- 会话管理（多轮对话上下文、历史会话列表、会话删除）
- 知识库定时同步 + 手动reindex端点
- 帖子增删改自动触发向量同步（Feign异步通知 + 定时补偿 + Resilience4j熔断）
- 双数据源架构（MySQL存业务表 + PostgreSQL pgvector存向量）
...
│   ├── 📂 campushare-agent/     # AI智能助手服务（端口8083，WebFlux响应式）
│   ├── 📂 campushare-gateway/   # API 网关（端口8080，Spring Cloud Gateway）
│   ├── 📂 docker/               # Docker 环境配置
│   │   ├── 📂 mysql/           # 数据库初始化脚本+迁移脚本
│   │   ├── 📂 postgres/        # PostgreSQL向量数据库初始化
...
│
├── 📂 docs/                     # 📚 项目文档
│   ├── 📂 agent-assistant/     # AI智能助手详细设计文档（93个文档/197个ADR）
│   ├── 📂 architecture-upgrade/ # 🔧 架构升级规划（后端/前端技术债务与升级路线）
│   ├── PRD.md                  # 产品需求文档
...
| MySQL | localhost:3306 | 🗄️ 业务数据库 | root / root123456 |
| PostgreSQL | localhost:5432 | 🧠 向量数据库（AI助手） | campushare / agent_pg_password |
...
| 文档 | 说明 |
|------|------|
| [AI智能助手设计文档](./docs/agent-assistant/) | 🤖 93份详细设计文档，涵盖架构/LLM策略/RAG/Agent设计/Prompt工程/前后端集成等 |
| [架构升级规划](./docs/architecture-upgrade/) | 🔧 前后端技术债务分析与分阶段升级路线（Nacos/RocketMQ/Sentinel/Redisson/TanStack Query等） |
