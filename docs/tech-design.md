...
前后端分离 + 微服务架构，当前已拆分为4个微服务+1个公共模块。
...
└───────────┬──────────────────┬─────────────────┬──────────┘
   ...
   │ - 注册登录     │F │ - 发帖/浏览    │F │ - SSE流式对话  │
   │ - 用户信息     │e │ - 评论/互动    │e │ - 知识库检索    │
   │ - 关注/私信    │i │ - 分类/子分类  │i │ - 会话管理      │
   │ - 通知/文件    │g │ - 点赞/收藏    │g │ - 工具调用      │
   │ - 创作者认证   │n │ - 浏览历史     │n │ - RAG检索增强   │
   │                │  │ - 数据初始化   │  │ (WebFlux响应式) │
   └────────┬───────┘  └───────┬────────┘  └──────┬─────────┘
            │                  │                  │
   ┌────────▼──────────────────▼────────┐  ┌──────▼─────────┐
   │          MySQL 8 / Redis 7         │  │ PostgreSQL 16  │
   │      (共享数据库，按表所有权划分)   │  │ + pgvector向量库│
   └───────────────────────────────────┘  └────────────────┘

服务间通信: OpenFeign双向调用（禁止跨服务直接访问数据库）
AI服务: SSE流式输出，混合检索（向量+全文），DeepSeek-V3 LLM
```

> **📋 架构升级规划**：当前架构正在进行专业升级，包括引入Nacos（服务发现/配置中心）、RocketMQ（消息队列）、Sentinel（熔断限流）、Redisson（分布式锁）、Elasticsearch（全文检索）、WebSocket（实时通信）等基础设施。详见 [architecture-upgrade/](./architecture-upgrade/) 目录。
