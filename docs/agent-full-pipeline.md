...
sequenceDiagram
    ...
    alt 快路径（OUT_OF_SCOPE / NAVIGATE）
        ...
        RAG->>LLM: embed(query) → 1024维(BGE-M3 via SiliconFlow API)
        par 四路并行（按意图动态topK）
...
flowchart TB
    ...
    subgraph RAG层["RAG混合检索（在线）"]
        direction TB
        Embed["Embedding<br/>BGE-M3(SiliconFlow) → 1024维<br/>熔断+重试+空向量降级"]
...
- **虚拟线程**：启用`spring.threads.virtual.enabled=true`，阻塞操作调度到虚拟线程提升吞吐量
...
```mermaid
flowchart TD
    Q["改写后的query"] --> Embed["BGE-M3(SiliconFlow) Embedding<br/>→ 1024维向量<br/>(熔断:空向量降级)"]
...
flowchart LR
    subgraph 外部["外部依赖"]
        ...
        PostSvc[("post-service :8082<br/>帖子数据WebClient调用<br/>分页拉取+增量通知")]
        UserSvc[("common模块<br/>JwtUtils解析userId")]
    end
...
(未来)agent.llm.*       LLM耗时/token/错误"]
    ...
    Chat --> PG
    Chat --> Redis
    Chat -->|WebClient| PostSvc
    Ingest -->|分页拉取WebClient| PostSvc
    Ingest -->|embed| DS
    Ingest --> MySQL
    Ingest --> PG
    Sched -->|定时触发| Chat
...
agent-service共4个Controller，25个端点，分为三类：用户端公开接口、内部服务接口、管理员接口。

### 8.5.1 用户端接口（AgentController + AgentCategoryController）

所有用户端接口需JWT鉴权（Authorization: Bearer xxx），返回统一`Result<T>`格式（SSE接口除外）。

**AgentController（`/agent`）**：

| HTTP方法 | 路径 | 功能 | 特殊说明 |
|---------|------|------|---------|
| POST | `/agent/chat` | SSE流式聊天 | produces=TEXT_EVENT_STREAM，error事件返回纯文本 |
| POST | `/agent/sessions` | 创建新会话 | body可选（默认标题"新对话"） |
| GET | `/agent/sessions` | 获取用户会话列表 | 排除DELETED状态，按lastMessageAt倒序 |
| GET | `/agent/sessions/{sid}` | 获取单个会话详情 | 校验会话所有权 |
| GET | `/agent/sessions/{sid}/turns` | 获取对话轮次历史 | 按turnNumber正序，含refs引用源 |
| PUT | `/agent/sessions/{sid}/archive` | 归档会话 | 触发记忆抽取+Redis延迟清理 |
| DELETE | `/agent/sessions/{sid}` | 删除会话 | 先归档再标记DELETED（软删除） |
| PUT | `/agent/sessions/{sid}/category` | 移动会话到分类 | 接受categoryId参数 |

**AgentCategoryController（`/agent/categories`）**：

| HTTP方法 | 路径 | 功能 |
|---------|------|------|
| POST | `/agent/categories` | 创建分类（名称唯一校验） |
| GET | `/agent/categories` | 获取用户所有分类 |
| PUT | `/agent/categories/{cid}` | 重命名分类 |
| DELETE | `/agent/categories/{cid}` | 删除分类（下属会话置为未分类） |

### 8.5.2 内部服务接口（InternalAgentController）

路径前缀`/internal/agent`，供post-service等内部服务调用，不经过网关，通过X-Internal-Token请求头鉴权。

| HTTP方法 | 路径 | 功能 |
|---------|------|------|
| POST | `/internal/agent/knowledge/reindex` | 手动触发知识库全量重建索引 |
| GET | `/internal/agent/knowledge/status` | 查看知识库索引状态（chunk数/article数） |
| GET | `/internal/agent/knowledge/{id}/versions` | 查询知识文章版本历史 |
| POST | `/internal/agent/knowledge/{id}/rollback` | 回滚知识文章到指定版本（?version=参数） |
| POST | `/internal/agent/knowledge/{id}/feedback` | 用户反馈点赞/点踩（?positive=参数） |
| GET | `/internal/agent/vector/status` | 查看向量库状态（知识库+帖子向量数量） |
| POST | `/internal/agent/posts/sync` | 接收post-service帖子变更通知 |
| POST | `/internal/agent/posts/reindex` | 手动触发帖子全量同步索引 |

### 8.5.3 管理员接口（PromptVersionController）

路径前缀`/agent/prompt-versions`，通过X-Internal-Token鉴权，用于System Prompt版本管理。

| HTTP方法 | 路径 | 功能 |
|---------|------|------|
| GET | `/agent/prompt-versions/current` | 查询当前生效Prompt版本（支持X-User-Id灰度测试） |
| GET | `/agent/prompt-versions/list` | 查询所有Prompt版本列表（按创建时间倒序） |
| POST | `/agent/prompt-versions/switch` | 切换到指定版本（body: version） |
| POST | `/agent/prompt-versions/rollback` | 回滚到上一个RELEASED版本 |
| POST | `/agent/prompt-versions/gray-ratio` | 设置灰度比例（?ratio=0-100） |

***

## 八点六、基础设施：双数据源与三WebClient连接池

### 8.6.1 双数据源隔离

agent-service同时连接MySQL（主业务库）和PostgreSQL/pgvector（向量库），两套数据源完全隔离：

| 数据源 | 前缀 | 用途 | ORM/访问方式 |
|--------|------|------|-------------|
| MySQL（@Primary） | `spring.datasource` | 业务表（sessions/turns/memory/prompt/knowledge） | MyBatis-Plus自动映射 |
| PostgreSQL/pgvector | `app.datasource.pgvector` | 向量表（knowledge_vectors/post_vectors/memory_vectors） | 独立JdbcTemplate + @Qualifier("pgvectorJdbcTemplate") |

**关键设计**：
- MySQL标记`@Primary`，MyBatis-Plus的MapperScan默认使用此数据源
- pgvector数据源通过`PgVectorConfig`手动创建独立DataSource和JdbcTemplate
- VectorStore类全部注入`@Qualifier("pgvectorJdbcTemplate")`，避免与MyBatis-Plus冲突
- pgvector HikariCP连接池配置：max=5, minIdle=1, connectionTimeout=10s（低并发场景，向量查询轻量）

### 8.6.2 三个独立WebClient连接池

三个外部HTTP调用各自独立配置WebClient，拥有独立连接池，互不影响：

| 配置类 | Bean名 | 目标服务 | 连接池大小 | 超时 | 特点 |
|--------|--------|---------|-----------|------|------|
| `WebClientConfig` | deepSeekWebClient | DeepSeek API | max=50 | connect=10s, response=60s | Bearer Token认证，SSE流式，codec 16MB |
| `EmbeddingWebClientConfig` | embeddingWebClient | SiliconFlow API（BGE-M3） | max=30 | connect=10s, response=30s | Bearer Token认证，批量向量化，codec 16MB |
| `PostWebClientConfig` | postWebClient | post-service:8082 | max=20 | connect=10s, response=30s | 无认证，内部服务调用，codec 2MB |

**注意**：Embedding使用**硅基流动（SiliconFlow）云端API**（`https://api.siliconflow.cn`），模型`BAAI/bge-m3`，非本地TEI服务。每个WebClient均配置了：
- Netty的`ReadTimeoutHandler`和`WriteTimeoutHandler`防止连接挂死
- `pendingAcquireTimeout`连接池等待超时
- Resilience4j熔断器隔离保护

### 8.6.3 时间自动填充

`MyMetaObjectHandler`实现MyBatis-Plus的`MetaObjectHandler`接口：
- insert时自动填充`createdAt`和`updatedAt`为当前时间（UTC+8）
- update时自动填充`updatedAt`
- 所有实体类通过`@TableField(fill = FieldFill.INSERT)`等注解声明

***

## 八点七、VectorStore层详解：三个向量存储的差异设计

文档在RAG和记忆章节提到了向量检索，但三个VectorStore类各有专属设计：

### 8.7.1 KnowledgeVectorStore（知识库向量）

**表**：`knowledge_vectors`

| 字段 | 说明 |
|------|------|
| article_id | 关联knowledge_articles |
| chunk_index | 分块序号（从0开始），一个文章对应多个chunk |
| chunk_text | 分块文本 |
| heading_path | 标题路径（如"平台指南 > 发帖 > 上传图片"） |
| content_md5 | 内容MD5用于增量检测 |
| embedding | 1024维向量（vector(1024)） |
| quality_score | 四维质量分（影响排序） |
| recall_count | 召回次数（log归一化影响质量分） |
| token_count | 该chunk的token数 |
| embedding_model | embedding模型标识 |

**关键设计**：
- **先删后插策略**：文档更新时先DELETE所有article_id的chunk，再INSERT新chunk。因为分块数可能变化，ON CONFLICT DO UPDATE无法清理多余旧chunk
- **双路检索**：`searchChunks()`向量HNSW余弦距离（`<=>`）+ `keywordSearchChunks()` pg_trgm GIN三元组模糊匹配
- **重复检测只查chunk_index=0**：用首个chunk作为代表性向量做相似度比较，避免N次查询
- **异步召回计数**：`incrementRecall()`使用boundedElastic线程池异步+1，不阻塞检索主流程
- **质量分更新**：用户点赞/点踩后通过`updateQualityScore()`实时更新

### 8.7.2 PostVectorStore（帖子向量）

**表**：`post_vectors`

与知识库向量最大不同——帖子支持**结构化slots过滤**：
- **school_name**：学校中文名（ILIKE模糊匹配），通过`SchoolNameUtils`归一化别名
- **category_name**：分类名（ILIKE模糊匹配）
- **post_type**：帖子类型（EXPERIENCE/RESOURCE/QUESTION等，等值匹配）
- 冗余字段：author_verified、like_count、view_count、comment_count

**关键设计**：
- **slots过滤检索**：向量和关键词检索均支持传入school/category/postType过滤条件，在SQL WHERE子句中执行
- **学校过滤降级**：按学校过滤无结果时，自动降级为无学校过滤重试（避免因别名匹配遗漏）
- **upsert语义**：`ON CONFLICT (post_id) DO UPDATE`，增量更新不重复插入
- **中文归一化**：所有学校名通过SchoolNameUtils归一化为标准中文名后再存储和查询

### 8.7.3 MemoryVectorStore（长期记忆向量）

**表**：`memory_vectors`

长期记忆采用**MySQL+PostgreSQL双存储**设计：
- MySQL（user_memory表）：结构化存储，支持CRUD、confidence累加、conflict_flag标记、衰减计算
- PostgreSQL（memory_vectors表）：向量存储，支持语义检索和衰减加权

**记忆类型**：6种类型（PREFERENCE/FACT/BEHAVIOR/TASK/SKILL/EVENT），检索时按decay_score加权（`similarity * decay_score`）

**关键设计**：
- **访问追踪**：`recordAccess()`记录access_count+1和last_accessed_at，近7天访问≥3次衰减减半
- **软删除**：`delete()`标记is_active=FALSE，`hardDelete()`物理删除（用于TASK类型4周清理）
- **画像加载**：`loadProfileMemories()`返回PREFERENCE+FACT类型Top-10高置信记忆，格式化为L1用户画像
- **中文标签**：`formatMemoryTitle()`将类型/key转为中文（如"偏好: 专业"、"事实: 学校"）

***

## 八点八、关键设计补充

### 8.8.1 命名规范：策略模式而非Impl后缀

知识处理三件套采用策略模式命名（按实现策略命名），不是接口+Impl后缀：

| 接口 | 实现类 | 命名含义 |
|------|--------|---------|
| KnowledgeChunker | MarkdownChunker | Markdown语义分块（未来可扩展LatexChunker/HtmlChunker） |
| KnowledgeDuplicateDetector | ThresholdDuplicateDetector | 双阈值相似度去重 |
| KnowledgeQualityScorer | FourDimensionQualityScorer | 四维质量评分 |

Service层同理：仅`AgentSessionService`和`AgentSessionCategoryService`有Impl后缀的实现类（`AgentSessionServiceImpl`、`AgentSessionCategoryServiceImpl`），其余20+个Service均为直接`@Service`具体类，无接口。

### 8.8.2 会话分类（AgentSessionCategoryService）

提供文件夹式会话管理：
- 用户可创建/重命名/删除分类
- 分类名称在用户范围内唯一
- 删除分类时，下属会话的categoryId置null（软转移，不级联删除）
- 支持移动会话到指定分类

### 8.8.3 配置前缀说明

所有自定义配置项统一使用`app.*`前缀（非`agent.*`），主要配置项：

| 配置路径 | 默认值 | 说明 |
|----------|--------|------|
| `app.llm.deepseek.base-url` | https://api.deepseek.com | DeepSeek API地址 |
| `app.llm.deepseek.model` | deepseek-v4-flash | LLM模型名 |
| `app.llm.deepseek.timeout-ms` | 60000 | LLM超时(ms) |
| `app.llm.embedding.base-url` | https://api.siliconflow.cn | Embedding API地址 |
| `app.llm.embedding.model` | BAAI/bge-m3 | Embedding模型名 |
| `app.llm.embedding.batch-size` | 32 | 批量向量化大小 |
| `app.retrieval.intent-driven.enabled` | true | 意图驱动检索开关 |
| `app.retrieval.cache.enabled` | true | 检索结果Redis缓存开关 |
| `app.retrieval.cache.ttl` | 5m | 缓存TTL |
| `app.rrf-k` | 60 | RRF融合k值 |
| `app.knowledge.chunk.target-tokens` | 256 | 分块目标token数 |
| `app.knowledge.chunk.max-tokens` | 512 | 分块最大token数 |
| `app.knowledge.chunk.overlap-tokens` | 50 | 相邻chunk重叠token数 |
| `app.knowledge.quality.weight-recall` | 0.4 | 质量分：召回权重 |
| `app.knowledge.quality.weight-feedback` | 0.3 | 质量分：反馈权重 |
| `app.knowledge.quality.weight-freshness` | 0.2 | 质量分：新鲜度权重 |
| `app.knowledge.quality.weight-completeness` | 0.1 | 质量分：完整度权重 |
| `app.knowledge.duplicate.threshold-exact` | 0.95 | 重复检测阈值 |
| `app.knowledge.duplicate.threshold-similar` | 0.85 | 相似检测阈值 |
| `app.internal.token` | (需配置) | 内部API鉴权Token |
| `app.prompt.version.current` | v1.0.0 | 当前Prompt版本 |
| `app.prompt.version.gray-ratio` | 100 | 灰度比例(0-100) |
| `spring.threads.virtual.enabled` | true | 虚拟线程开关 |

### 8.8.4 SemVer版本工具

`SemVer`工具类实现语义化版本号（vMAJOR.MINOR.PATCH）：
- `parse()`/`parseOrInitial()`：解析版本字符串，失败返回v1.0.0
- `nextPatch()`/`nextMinor()`/`nextMajor()`：版本递增
- `compareTo()`：版本比较
- 不可变对象，线程安全
- 同时用于知识库版本和Prompt版本管理

### 8.8.5 错误事件格式

SSE error事件的data字段为**纯文本字符串**（非JSON），例如：
- `"请求过于频繁，每分钟最多 10 次，请稍后再试"`
- `"服务异常，请稍后重试"`
- e.getMessage() 异常消息
...
| 模块                | 核心文件                                                                                                                                                                           | 主要职责                                 |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------ |
| **入口/Controller** | [AgentController.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/controller/AgentController.java)                      | SSE聊天端点、会话CRUD API、轮次查询、归档/删除/分类移动    |
| **分类Controller**  | [AgentCategoryController.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/controller/AgentCategoryController.java)      | 会话分类CRUD API                         |
| **内部Controller**  | [InternalAgentController.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/controller/InternalAgentController.java)      | 知识库重建/状态/回滚/反馈、向量状态、帖子同步通知等内部API   |
| **Prompt管理Controller** | [PromptVersionController.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/controller/PromptVersionController.java)    | Prompt版本查询/切换/回滚/灰度设置              |
| **限流**            | [AgentRateLimiter.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/AgentRateLimiter.java)                       | Redis固定窗口限流(10次/分/userId)、fail-open |
| **弹性配置**          | [ResilienceConfig.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/config/ResilienceConfig.java)                        | 4个熔断器(deepseek/embedding/post-sync/intent-classifier)+1个全局限流器 |
...
| **会话管理**          | [AgentSessionService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/AgentSessionService.java)                 | 会话CRUD、权限验证、轮次查询、引用列表，实现类AgentSessionServiceImpl |
| **会话管理实现**       | [AgentSessionServiceImpl.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/AgentSessionServiceImpl.java)         | 会话服务实现：Redis+DB双写、初始化短期记忆5Key         |
...
| **会话分类**          | [AgentSessionCategoryService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/AgentSessionCategoryService.java) | 文件夹分类CRUD接口，实现类AgentSessionCategoryServiceImpl |
| **会话分类实现**       | [AgentSessionCategoryServiceImpl.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/AgentSessionCategoryServiceImpl.java) | 分类管理实现：名称唯一校验、删除时软转移              |
...
| **LLM分类**         | [IntentClassifier.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/IntentClassifier.java)                       | Layer2 LLM结构化意图分类+查询改写+槽位提取(三合一省500ms) |
...
| **意图缓存**          | [IntentCacheService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/IntentCacheService.java)                   | 相同query意图结果缓存(TTL=1h)                |
| **意图路由**          | [IntentRouter.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/IntentRouter.java)                               | 快/慢路径分流+OUT_OF_SCOPE/NAVIGATE模板回复选择   |
| **RAG检索**         | [RetrievalService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/RetrievalService.java)                       | 四路并行+CLARIFY合并→RRF融合→质量加权→跨源去重→Token截断→Redis缓存 |
| **Prompt装配**      | [PromptAssembler.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/prompt/PromptAssembler.java)                          | L1平台+L2任务+L3 Few-shot+<context>+L4护栏五层拼接 |
| **Prompt版本管理**    | [PromptVersionManager.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/prompt/PromptVersionManager.java)                | 版本灰度切换(userId哈希分流)、Redis缓存、Redis→DB→硬编码三级降级 |
...
| **上下文组装**         | [ContextAssembler.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/ContextAssembler.java)                       | L0-L5分层装载+Token预算(~8000)+三级降级链+XML标签分层  |
| **上下文快照**         | [ContextSnapshotService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/ContextSnapshotService.java)           | fire-and-forget异步写入agent_context_snapshots快照，答错回放证据 |
| **短期记忆**          | [ConversationMemoryService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/ConversationMemoryService.java)     | Redis 5Key读写(meta/messages/summary/slots/pinned)、TTL=2h滑动续期、MySQL回源回写 |
| **上下文压缩**         | [ContextCompressionService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/ContextCompressionService.java)     | 三合一LLM压缩(摘要≤300字+槽位+Pin)、失败降级LTRIM     |
| **长期记忆**          | [LongTermMemoryService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/LongTermMemoryService.java)             | 记忆抽取/UPSERT(confidence累加)/周衰减/向量化，MySQL+PG双存储 |
| **冲突解决**          | [ConflictResolver.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/ConflictResolver.java)                       | EXPLICIT优先冲突解决、Jaccard<0.3值冲突判定、审计记录写入history表 |
| **记忆检索**          | [MemoryRetrievalService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/MemoryRetrievalService.java)           | 用户画像加载(PREFERENCE+FACT Top10)+相关记忆双路RRF检索 |
| **知识库摄入**         | [KnowledgeIngestionService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/KnowledgeIngestionService.java)     | MD5增量→frontmatter解析→分块→embedding批量32→去重→质量评分→版本快照→MySQL+PG写入 |
| **分块接口**          | [KnowledgeChunker.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/KnowledgeChunker.java)                       | 分块策略接口                            |
| **Markdown分块**    | [MarkdownChunker.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/MarkdownChunker.java)                         | H2→H3→段落→句子四级语义分块、50token重叠、headingPath、embeddingText拼接 |
| **去重接口**          | [KnowledgeDuplicateDetector.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/KnowledgeDuplicateDetector.java)   | 重复检测策略接口                            |
| **重复检测**          | [ThresholdDuplicateDetector.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/ThresholdDuplicateDetector.java)   | 双阈值检测(0.95 DUPLICATE/0.85 SIMILAR)、只查chunk_index=0、fail-open返回UNIQUE |
| **质量评分接口**        | [KnowledgeQualityScorer.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/KnowledgeQualityScorer.java)           | 质量评分策略接口                            |
| **质量评分**          | [FourDimensionQualityScorer.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/FourDimensionQualityScorer.java)   | 四维加权评分(召回0.4+反馈0.3+新鲜0.2+完整0.1)、log归一化、线性衰减 |
| **知识版本**          | [KnowledgeVersionService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/KnowledgeVersionService.java)         | 完整快照(非diff)、SemVer递增、回滚产生新版本、软下线(deprecate)、反馈调整质量分 |
| **帖子向量同步**        | [PostVectorService.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/service/PostVectorService.java)                     | WebClient调用post-service分页拉取+通知增量、独立熔断、title+excerpt拼接embed |
| **知识向量存储**        | [KnowledgeVectorStore.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/store/KnowledgeVectorStore.java)                 | knowledge_vectors CRUD、向量/关键词双路检索、先删后插、异步召回计数 |
| **帖子向量存储**        | [PostVectorStore.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/store/PostVectorStore.java)                           | post_vectors CRUD、slots过滤检索(school/category ILIKE)、学校过滤降级 |
| **记忆向量存储**        | [MemoryVectorStore.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/store/MemoryVectorStore.java)                       | memory_vectors CRUD、decay_score加权、访问追踪、软删除、画像加载 |
| **LLM客户端**        | [DeepSeekClient.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/llm/DeepSeekClient.java)                               | DeepSeek API封装（流式SSE+非流式+熔断+重试）         |
| **Embedding客户端**  | [EmbeddingClient.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/llm/EmbeddingClient.java)                             | SiliconFlow BGE-M3批量向量化、自动分片32、Mono.zip并行、熔断空向量降级 |
| **DeepSeek WebClient** | [WebClientConfig.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/config/WebClientConfig.java)                           | deepseek连接池(max=50)、超时60s、Bearer认证、SSE流式 |
| **Embedding WebClient** | [EmbeddingWebClientConfig.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/config/EmbeddingWebClientConfig.java)         | siliconflow连接池(max=30)、超时30s、Bearer认证  |
| **Post WebClient** | [PostWebClientConfig.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/config/PostWebClientConfig.java)                     | post-service连接池(max=20)、超时30s、无认证       |
| **pgvector配置**   | [PgVectorConfig.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/config/PgVectorConfig.java)                           | 双数据源配置：MySQL(@Primary)+PostgreSQL/pgvector独立JdbcTemplate |
| **自动填充**          | [MyMetaObjectHandler.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/config/MyMetaObjectHandler.java)                   | MyBatis-Plus自动填充createdAt/updatedAt(UTC+8) |
| **安全护栏**          | [ConstitutionalAIValidator.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/prompt/ConstitutionalAIValidator.java)      | 输入硬拦截(shouldHardBlock)+注入软检测(detectInjection)+输出违规验证 |
| **学校工具**          | [SchoolNameUtils.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/util/SchoolNameUtils.java)                            | 13所高校ID映射+别名归一化(北大→北京大学等)+query正则提取 |
| **版本工具**          | [SemVer.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/util/SemVer.java)                                               | 语义化版本号解析/比较/递增(patch/minor/major)、不可变线程安全 |
| **Token计数**       | [TokenCounter.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/util/TokenCounter.java)                                   | jtokkit cl100k_base编码计数+truncateToTokens截断 |
| **意图指标**          | [IntentMetricsConfig.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/config/IntentMetricsConfig.java)                  | 意图分类Micrometer埋点(分类计数/耗时/缓存命中/路由决策) |
| **知识指标**          | [KnowledgeMetricsConfig.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/config/KnowledgeMetricsConfig.java)            | 知识库摄入/分块/embedding/检索/去重指标、@PostConstruct预创建防重复注册 |
| **知识调度**          | [KnowledgeScheduler.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/config/KnowledgeScheduler.java)                    | 启动30s+每小时触发ingestAll（MD5增量，未变更跳过） |
| **帖子调度**          | [PostVectorScheduler.java](file:///e:/workspace_work/CampusShare/backend/campushare-agent/src/main/java/com/campushare/agent/config/PostVectorScheduler.java)                  | 启动60s+每5分钟触发syncAll兜底（通知丢失也能补齐） |
| **前端页面**          | [AgentPage.tsx](file:///e:/workspace_work/CampusShare/frontend/src/pages/AgentPage.tsx)                                                                                        | 聊天UI+SSE消费(ReadableStream)+流式Markdown渲染+引用卡片+跳转卡片+输入框重聚焦 |

***

*本文档描述的是2026-07-11代码现状。架构设计文档见* *[docs/agent-assistant/architecture-overview.md](file:///e:/workspace_work/CampusShare/docs/agent-assistant/architecture-overview.md)（包含演进路线图），各模块详细ADR见* *`docs/agent-assistant/`* *下93份设计文档。*
