...
function ToggleRow({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-50 last:border-0">
      ...
      >
        <span
          className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${currentChecked ? 'translate-x-5' : ''}`}
        />
      </button>
    </div>
  )
...
export default function SettingsPage() {
  ...
  const faqs = [
    {
      q: '如何发布帖子？',
      a: '进入学校社区页面，点击右下角"+"按钮，选择帖子类型并填写内容即可发布。',
    },
    {
      q: '如何修改个人资料？',
      a: '在个人主页点击头像旁的编辑按钮，可修改昵称和个人简介。点击头像可上传新头像。',
    },
    { q: '帖子可以删除吗？', a: '可以。在帖子详情页，作者可以点击右上角删除按钮删除自己的帖子。' },
    {
      q: '评论可以被删除吗？',
      a: '评论作者和帖子作者都可以删除评论。评论作者删除后，该评论从"我的回复"中移除。',
    },
    ...
    <div className="min-h-screen bg-gray-50 pb-16">
      ...
      <div className="max-w-5xl mx-auto px-4 py-4">
          ...
          <div className="space-y-3">
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-50">
                <p className="text-xs font-medium text-gray-400">账号信息</p>
              </div>
              ...
              <button
                onClick={() => {
                  setOriginalAccount(currentUser?.email || '')
                  setOriginalVerifyCode('')
                  setNewAccount('')
                  setNewVerifyCode('')
                  setRebindStep(currentUser?.email ? 1 : 2)
                  setModal('email')
                }}
              ...
              <button
                onClick={() => {
                  setOriginalAccount(currentUser?.phone || '')
                  setOriginalVerifyCode('')
                  setNewAccount('')
                  setNewVerifyCode('')
                  setRebindStep(currentUser?.phone ? 1 : 2)
                  setModal('phone')
                }}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-50">
                <p className="text-xs font-medium text-gray-400">安全设置</p>
              </div>
              <button
                onClick={() => {
                  setOldPassword('')
          ...
          >
            <div className="bg-white w-72 rounded-2xl p-5" onClick={(e) => e.stopPropagation()}>
              ...
              <input
                ...
                placeholder="旧密码"
                className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white mb-3"
              />
              <input
                type="password"
                value={newPassword}
                onChange={(e) => setNewPassword(e.target.value)}
                placeholder="新密码（6-20位，含字母数字）"
                className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white mb-3"
              />
              <input
                type="password"
                ...
                placeholder="确认新密码"
                className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white mb-4"
              />
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={saving}
                  className="flex-1 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition-colors disabled:opacity-40"
                >
                  {saving ? '保存中...' : '确认'}
                </button>
              </div>
              ...
              {rebindStep === 1 ? (
                <>
                  ...
                  <div className="flex gap-2 mb-4">
                    <input
                      type="text"
                      value={originalVerifyCode}
                      onChange={(e) => setOriginalVerifyCode(e.target.value)}
                      placeholder="验证码"
                      className="flex-1 px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white"
                    />
                    <button
                      onClick={() => handleSendCode(originalAccount, 'email')}
                      disabled={sendingCode}
                      className="px-3 py-2.5 bg-gray-100 text-gray-600 rounded-xl text-xs font-medium hover:bg-gray-200 transition-colors disabled:opacity-40 whitespace-nowrap"
                    >
                      {sendingCode ? '发送中' : '发送'}
                    </button>
                  </div>
                  <div className="flex gap-3">
                    ...
                    <button
                      onClick={() => {
                        if (originalVerifyCode) setRebindStep(2)
                        else toast.warning('请输入验证码')
                      }}
                ...
                <>
                  ...
                  <div className="flex gap-2 mb-4">
                    <input
                      type="text"
                      value={newVerifyCode}
                      onChange={(e) => setNewVerifyCode(e.target.value)}
                      placeholder="验证码"
                      className="flex-1 px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white"
                    />
                    <button
                      onClick={() => handleSendCode(newAccount, 'email')}
                      disabled={sendingCode}
                      className="px-3 py-2.5 bg-gray-100 text-gray-600 rounded-xl text-xs font-medium hover:bg-gray-200 transition-colors disabled:opacity-40 whitespace-nowrap"
                    >
                      {sendingCode ? '发送中' : '发送'}
              ...
              {rebindStep === 1 ? (
                <>
                  ...
                  <div className="flex gap-2 mb-4">
                    ...
                    />
                    <button
                      onClick={() => handleSendCode(originalAccount, 'phone')}
                      disabled={sendingCode}
                      className="px-3 py-2.5 bg-gray-100 text-gray-600 rounded-xl text-xs font-medium hover:bg-gray-200 transition-colors disabled:opacity-40 whitespace-nowrap"
                    >
                      {sendingCode ? '发送中' : '发送'}
                ...
                <>
                  <h3 className="text-base font-bold text-gray-900 mb-4">
                    {currentUser?.phone ? '绑定新手机号' : '绑定手机号'}
                  </h3>
                  <input
                    type="text"
                    value={newAccount}
                  ...
                  <div className="flex gap-2 mb-4">
                    ...
                    />
                    <button
                      onClick={() => handleSendCode(newAccount, 'phone')}
                      disabled={sendingCode}
                      className="px-3 py-2.5 bg-gray-100 text-gray-600 rounded-xl text-xs font-medium hover:bg-gray-200 transition-colors disabled:opacity-40 whitespace-nowrap"
                    >
                      {sendingCode ? '发送中' : '发送'}
            ...
            {!privacy ? (
              ...
              <>
                <ToggleRow
                  ...
                  label="公开我的帖子"
                  checked={privacy.publicPosts}
                  onChange={(v) => handlePrivacyToggle('publicPosts', v)}
                  disabled={updatePrivacy.isPending}
                />
                <ToggleRow
                  icon={<Eye className="w-4 h-4 text-gray-400" />}
                  label="公开我的收藏"
                  checked={privacy.publicStars}
                  onChange={(v) => handlePrivacyToggle('publicStars', v)}
                  disabled={updatePrivacy.isPending}
                />
                <ToggleRow
                  icon={<Eye className="w-4 h-4 text-gray-400" />}
                  label="公开我的点赞"
                  checked={privacy.publicLikes}
                  onChange={(v) => handlePrivacyToggle('publicLikes', v)}
                  disabled={updatePrivacy.isPending}
                />
                <ToggleRow
                  icon={<Eye className="w-4 h-4 text-gray-400" />}
                  ...
                  onChange={(v) => handlePrivacyToggle('publicHistory', v)}
                  disabled={updatePrivacy.isPending}
                />
                <ToggleRow
                  icon={<Eye className="w-4 h-4 text-gray-400" />}
                  ...
                  onChange={(v) => handlePrivacyToggle('searchable', v)}
                  disabled={updatePrivacy.isPending}
                />
              </>
            )}
        ...
        {settingsType === 'general' && (
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-50">
              <p className="text-xs font-medium text-gray-400">通用设置</p>
            </div>
            {!notif ? (
              <div className="text-center py-8">
                <div className="w-6 h-6 border-2 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto"></div>
              ...
              <>
                <ToggleRow
                  ...
                  label="新消息通知"
                  checked={notif.notifyMessages}
                  onChange={(v) => handleNotifToggle('notifyMessages', v)}
                  disabled={updateNotificationSettings.isPending}
                />
                <ToggleRow
                  icon={<Bell className="w-4 h-4 text-gray-400" />}
                  label="帖子回复通知"
                  checked={notif.notifyReplies}
                  onChange={(v) => handleNotifToggle('notifyReplies', v)}
                  disabled={updateNotificationSettings.isPending}
                />
                <ToggleRow
                  icon={<Bell className="w-4 h-4 text-gray-400" />}
                  label="点赞收藏通知"
                  checked={notif.notifyLikes}
                  onChange={(v) => handleNotifToggle('notifyLikes', v)}
                  disabled={updateNotificationSettings.isPending}
                />
              </>
            )}
        ...
        {settingsType === 'help' && (
          <div className="space-y-3">
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-50">
                <p className="text-xs font-medium text-gray-400">常见问题</p>
              </div>
              {faqs.map((faq, idx) => (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-50">
                <p className="text-xs font-medium text-gray-400">意见反馈</p>
              </div>
              <div className="p-4">
                <textarea
                  value={feedback}
