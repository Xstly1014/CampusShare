...
const configMap: Record<
  ListType,
  { title: string; icon: React.ReactNode; fetcher: () => Promise<any> }
> = {
  following: {
    title: '关注',
    icon: <UserPlus className="w-5 h-5" />,
    fetcher: userApi.getFollowingList,
  },
  followers: {
    title: '粉丝',
    icon: <Users className="w-5 h-5" />,
    fetcher: userApi.getFollowerList,
  },
  mutual: {
    title: '互相关注',
    icon: <UserCheck className="w-5 h-5" />,
    fetcher: userApi.getMutualList,
  },
}
...
export default function FollowListPage() {
  ...
  const listType: ListType = ['following', 'followers', 'mutual'].includes(pathSegment)
    ? pathSegment
    : 'following'
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {users.map((u) => (
              <div
                key={u.id}
                onClick={() => navigate(u.id === currentUser?.id ? '/profile' : `/user/${u.id}`)}
