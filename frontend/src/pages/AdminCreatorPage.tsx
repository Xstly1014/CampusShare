...
import { ChevronLeft, Check, X, Clock, Shield } from 'lucide-react'
import type { CreatorApplication } from '../services/api'
...
export default function AdminCreatorPage() {
  ...
  const [rejectModal, setRejectModal] = useState<{ id: number; visible: boolean }>({
    id: 0,
    visible: false,
  })
  ...
  const handleReject = async () => {
    ...
    try {
      await creatorApi.verifyApplication(rejectModal.id, {
        approved: false,
              ...
              <div key={app.id} className="bg-white rounded-2xl border border-gray-100 p-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center flex-shrink-0 overflow-hidden">
                      {app.avatarUrl ? (
                        <img
                          src={
                            app.avatarUrl.startsWith('/files/')
                              ? `/api${app.avatarUrl}`
                              : app.avatarUrl
                          }
                          alt=""
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        申请时间：
                        {app.applyTime ? new Date(app.applyTime).toLocaleString('zh-CN') : '-'}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 rounded-xl p-3 space-y-2 mb-3">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-gray-900 font-mono text-xs">
                      {maskIdCard(app.idCard)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500">总获赞数</span>
                    <span className="text-gray-900 font-medium">
                      {app.totalLikes?.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                  ...
                  {app.status === 'REJECTED' && app.rejectReason && (
                    <div className="flex items-start justify-between text-sm pt-2 border-t border-gray-200">
                      ...
                      <span className="text-red-600 text-right max-w-[60%]">
                        {app.rejectReason}
                      </span>
                    </div>
                  )}
                  {app.status === 'APPROVED' && app.reviewTime && (
                    <div className="flex items-center justify-between text-sm pt-2 border-t border-gray-200">
                      ...
                      <span className="text-green-600">
                        {new Date(app.reviewTime).toLocaleString('zh-CN')}
                      </span>
                    </div>
                  )}
                </div>
                ...
                {app.status === 'PENDING' && (
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => {
                        setRejectModal({ id: app.id, visible: true })
                        setRejectReason('')
                      }}
                      disabled={verifying === app.id}
