...
public interface PostDownloadMapper extends BaseMapper<PostDownload> {
    ...
    @Select("SELECT COUNT(*) FROM post_downloads pd INNER JOIN posts p ON pd.post_id = p.id " +
            "WHERE pd.user_id = #{userId} AND p.deleted = 0")
    long countValidByUserId(@Param("userId") String userId);

    @Select("SELECT pd.post_id FROM post_downloads pd INNER JOIN posts p ON pd.post_id = p.id " +
            "WHERE pd.user_id = #{userId} AND p.deleted = 0 " +
    ...
    @Select("SELECT p.category_id AS categoryId, COUNT(*) AS cnt FROM post_downloads pd " +
            ...
            "WHERE pd.user_id = #{userId} AND p.deleted = 0 AND p.category_id IS NOT NULL " +
            "GROUP BY p.category_id")
    List<Map<String, Object>> countDownloadsGroupByCategory(@Param("userId") String userId);

    @Select("SELECT p.school_id AS schoolId, COUNT(*) AS cnt FROM post_downloads pd " +
            "INNER JOIN posts p ON pd.post_id = p.id " +
            "WHERE pd.user_id = #{userId} AND p.deleted = 0 AND p.school_id IS NOT NULL AND p.category_id IS NULL " +
            "GROUP BY p.school_id")
    List<Map<String, Object>> countDownloadsGroupBySchool(@Param("userId") String userId);
}
